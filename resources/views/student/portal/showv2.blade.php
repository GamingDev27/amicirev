																		...
																		@if(isset($albums[$class->id]))
																			...
																			<div class="container-fluid p-0" >
																				<ul class="list-unstyled video-list-thumbs row  flex-row flex-nowrap overflow-auto m-0 p-1" style="background:white;border:solid 1px #212529;border-bottom:solid 4px #212529;">
																					@if(isset($albums[$class->id]["body"]) && isset($albums[$class->id]["body"]["data"]) && count($albums[$class->id]["body"]["data"]))
																						@foreach($albums[$class->id]["body"]["data"] as $video)
																							<li class="col-lg-3 col-sm-4 col-xs-6 col-6 p-0">
																								<a onclick="playVid('{{ $class->id}}','{{ str_replace('/videos/','',$video['uri'])}}');" title="{{ $video['name']}}" >
																									@php
																										$link = "";
																									...
																									@endphp
																									<img src="{{ $link}}" alt="Barca" class="img-responsive" width="100%" />
																									<h2>{{ $video['name']}}</h2>
																									<i class="glyphicon-play-circle fa fa-play-circle"></i>
																									<span class="duration">{{gmdate("H:i:s",$video['duration'])}}</span>
																								</a>
																							</li>
																						@endforeach
																					@endif
