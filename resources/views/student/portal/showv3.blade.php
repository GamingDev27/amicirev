...
<div class="container-fluid">
	...
	<div class="row">
		<div class="col-10">
			<b>DATE:</b> <em>{{ date('F d,Y',strtotime($batch->date_start)) }} to {{ date('F
				d,Y',strtotime($batch->date_end)) }}</em>
		</div>
	...
	<div class="blackred" width="100%">
		<nav>
			...
			<div class="tab-content py-3 px-3 px-sm-0" id="nav-tabContent">
				...
				@foreach($courses as $course)
					@if(isset($course->hascoach) && $course->hascoach)
						<div class="tab-pane fade {{$firstcourse?' active show':''}}" id="nav-{{$course->code}}" role="tabpanel"
					aria-labelledby="nav-{{$course->code}}-tab">
					@php
					$firstcourse = false;
					@endphp
					<p class="m-0 p-0 text-center" style="border-bottom:solid 5px lightgray">{{$course->code}} -
						{{$course->name}}</p>
					<div class="row">
						<div class="col-sm-5 col-lg-3">
							<div class="nav subjects-tab flex-column nav-pills p-1" id="v-pills-tab" role="tablist"
								aria-orientation="vertical">
								@foreach($classes[$course->id] as $class)
								@if($class->coach)
								<a class="nav-link subject-pill" id="v-pills-{{$class->subject_id}}-tab"
									data-coach-id="{{$class->id}}-{{$class->coach->id}}" data-toggle="pill"
									href="#v-pills-{{$class->subject_id}}" role="tab"
									aria-controls="v-pills-{{$class->subject_id}}" aria-selected="true">
									{{$class->subject->code}}
								</a>
								@endif
								@endforeach
							...
							<div>
								@foreach($classes[$course->id] as $class)
								@if($class->coach)
								<div class="card coaches-dv shadow m-2 bg-white rounded" style="display:none;"
									id="coach-{{$class->id}}-{{$class->coach->id}}-dv">
						...
						<div class="col-sm-7 col-lg-9">
							<div class="tab-content highlight h-100 p-1" id="v-pills-tabContent">
								@foreach($classes[$course->id] as $class)
								<div class="tab-pane fade" id="v-pills-{{$class->subject_id}}" role="tabpanel"
									aria-labelledby="v-pills-{{$class->subject_id}}-tab">
										...
										<div class="row align-items-stretch mb-5">
											<div class="col-md-12">
												<small>
													...
													{{ date('F d, Y', strtotime($class->date_start)) }} to {{ date('F d,
													Y', strtotime($class->date_end)) }}
													@endif
												...
												<div class="container-fluid p-0">
													<ul class="list-unstyled video-list-thumbs row  flex-row flex-nowrap overflow-auto m-0 p-1"
														style="background:white;border:solid 1px #212529;border-bottom:solid 4px #212529;">
														@if(isset($albums[$class->id]["items"]) &&
														count($albums[$class->id]["items"]))
														@foreach($albums[$class->id]["items"] as $video)
														<li class="col-lg-3 col-sm-4 col-xs-6 col-6 p-0">
															<a onclick="playVid('{{ $class->id}}','{{$video['guid']}}');"
																title="{{ $video['title']}}">
																@php
																$link = "";

																@endphp
																<img src="{{ $pullzone }}/{{ $video['guid'] }}/{{ $video['thumbnailFileName'] }}"
																	alt="Barca" class="img-responsive" width="100%" />
																<h2>{{ strtoupper($video['title']) }}</h2>
																<i class="glyphicon-play-circle fa fa-play-circle"></i>
																<span
																	class="duration">{{gmdate("H:i:s",$video['length'])}}</span>
															</a>
														</li>
														@endforeach
														@endif
												...
												<div class="m-2 p-1 border rounded">
													<div class="buttonshere">
														...
														<button class="btn btn-primary m-1"
															onclick="readp(this,'{{ $attachment->token }}')">
												...
												@endif
												@else
												<span class="alert alert-info m-2 p-0"> Please enroll to view Videos and
													PDFs</span>
												@endif
												@else
	...
	<div class="p-1">
		...
		@endif
		@else
		<a class="btn btn-success w-25 m-1 p-0" onclick="event.preventDefault();
